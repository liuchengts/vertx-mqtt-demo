  ...
  public void handle(JsonObject jsonObject) {
    ...
    if (dataJson.getBoolean(KeyConstant.PAC_DEL)) {
      ...
      try {
        ...
        String httpProxy = config.getNetworkIp() + ":" + dataJson.getLong(KeyConstant.PAC_HTTP_PORT);
        String socksProxy = config.getNetworkIp() + ":" + dataJson.getLong(KeyConstant.PAC_SOCKS_PORT);
        fileList.remove(1);
        fileList.remove(1);
